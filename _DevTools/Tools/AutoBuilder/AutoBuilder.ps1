...
task Build {
	...
	foreach ($build in $builds)
	{
		...
		Write-Host -ForegroundColor Green "Buildinaaaase " $buildParams
		
		if ($buildDebugAndRelease -eq $true)
		{
			Write-Host -ForegroundColor Green "Building Debug " $name
			Write-Host
			exec { msbuild "/t:Clean;Rebuild" "/p:Configuration=Debug" "/p:Platform=Any CPU" "/p:OutputPath=bin\Debug\" "/p:TreatWarningsAsErrors=$treatWarningsAsErrors" "$buildParams" "$defineConstants" ".\$name.sln" | Out-Default } "Error building $name"
		}
		
		Write-Host -ForegroundColor Green "Building Release " $name
		Write-Host
		exec { msbuild "/t:Clean;Rebuild" "/p:Configuration=Release" "/p:Platform=Any CPU" "/p:OutputPath=bin\Release\" "/p:TreatWarningsAsErrors=$treatWarningsAsErrors" "$buildParams" "$defineConstants" ".\$name.sln" | Out-Default } "Error building $name"
	}
