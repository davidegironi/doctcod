...
namespace DG.DoctcoD.Service
{
    public partial class ServiceWorker
    {
        ...
        private DoctcoDSerLanguage language = new DoctcoDSerLanguage();
        private const string LanguageFilenamePrefix = "DoctcoDSer-";
        ...
        /// </summary>
        private class DoctcoDSerLanguage
        {
            public string dateFormat = "yyyy/MM/dd";
            public string timeFormat = "HH:mm";
            public string sendAppointmentsReminderSubject = "Appointment reminder from Doctor Clinic";
            public string sendAppointmentsReminderBody = "Dear <b>%NAME%</b><br/><br/>This is a reminder that you have an appoinment with us on <b>%DATE%</b> at <b>%TIME%</b>.<br/><br/>Thank you,<br/><b>Doctor Clinic</b><br/><br/><br/><small>This message is a PRIVATE communication. This message and all attachments are a private communication and may be confidential or protected by privilege. If you are not the intended recipient, you are hereby notified that any disclosure, copying, distribution or use of the information contained in or attached to this message is strictly prohibited. Please notify the sender of the delivery error by replying to this message, and then delete it from your system. Thank you.</small>";
        }
        ...
        public ServiceWorker()
        {
...
#if DEBUG
            if (File.Exists("DoctcoD.exe.config"))
                configMap.ExeConfigFilename = "DoctcoD.exe.config";
            else
            {
                if (File.Exists("..\\..\\..\\DoctcoD\\bin\\Debug\\DoctcoD.exe.config"))
                    configMap.ExeConfigFilename = "..\\..\\..\\DoctcoD\\bin\\Debug\\DoctcoD.exe.config";
                else
            ...
            if (!String.IsNullOrEmpty(languageFolder + "\\" + LanguageFilenamePrefix + config.AppSettings.Settings["language"].Value + ".json"))
            {
                try
                {
                    ...
                    {
                        if (File.Exists("..\\..\\..\\DoctcoD\\bin\\Debug\\" + languageFolder + "\\" + LanguageFilenamePrefix + config.AppSettings.Settings["language"].Value + ".json"))
                            jsontext = File.ReadAllText("..\\..\\..\\DoctcoD\\bin\\Debug\\" + languageFolder + "\\" + LanguageFilenamePrefix + config.AppSettings.Settings["language"].Value + ".json");
                    }
#else
                    if (File.Exists(languageFolder + "\\" + LanguageFilenamePrefix + config.AppSettings.Settings["language"].Value + ".json"))
                        jsontext = File.ReadAllText(languageFolder + "\\" + LanguageFilenamePrefix + config.AppSettings.Settings["language"].Value + ".json");
#endif
                    language = new JavaScriptSerializer().Deserialize<DoctcoDSerLanguage>(jsontext);
                }
                catch { }
            }
            ...
            _doctcodModel.LanguageHelper.LoadFromFile(languageFolder + "\\" + language + ".json");
