...
{
    public class TaxesDeductionsRepository : GenericDataRepository<taxesdeductions, DoctcoDModel>
    {
        ...
        private bool Validate(bool isUpdate, ref string[] errors, params taxesdeductions[] items)
        {
            ...
            foreach (taxesdeductions item in items)
            {
                ...
                if (!isUpdate)
                {
                    if (Any(r => r.taxesdeductions_name == item.taxesdeductions_name))
                    {
                        ret = false;
                ...
                {
                    if (Any(r => r.taxesdeductions_id != item.taxesdeductions_id && r.taxesdeductions_name == item.taxesdeductions_name))
                    {
                        ret = false;
                        errors = errors.Concat(new string[] { language.text002 }).ToArray();
                    }
