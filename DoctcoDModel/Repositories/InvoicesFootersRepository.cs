...
{
    public class InvoicesFootersRepository : GenericDataRepository<invoicesfooters, DoctcoDModel>
    {
        ...
        private bool Validate(bool isUpdate, ref string[] errors, params invoicesfooters[] items)
        {
            ...
            foreach (invoicesfooters item in items)
            {
                ...
                if (!isUpdate)
                {
                    if (Any(r => r.invoicesfooters_name == item.invoicesfooters_name))
                    {
                        ret = false;
                ...
                {
                    if (Any(r => r.invoicesfooters_id != item.invoicesfooters_id && r.invoicesfooters_name == item.invoicesfooters_name))
                    {
                        ret = false;
                        errors = errors.Concat(new string[] { language.text003 }).ToArray();
                    }
